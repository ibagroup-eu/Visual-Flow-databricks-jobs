...
class DatabricksNativeWriteStage(override val id: String, config: DatabricksNativeStageConfig, options: Map[String, String])
  extends WriteStage(id, DatabricksNativeStageConfig.storageId){
...
object DatabricksNativeWriteStageBuilder extends WriteStageBuilder {
  ...
  override protected def convert(config: Node): Stage =
    new DatabricksNativeWriteStage(config.id, new DatabricksNativeStageConfig(config), getOptions(config.value))
}