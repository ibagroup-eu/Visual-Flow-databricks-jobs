...
object ObjectStorageWriteS3StageBuilder extends WriteStageBuilder {
  ...
  override protected def convert(config: Node): Stage = {
    ...
    new ObjectStorageWriteStage(config.id, this, s3Config, getOptions(config.value) ++ s3Config.options, S3Config.cosStorage)
  }
}
...
object AzureBlobStorageWriteStageBuilder extends WriteStageBuilder {
  ...
  override protected def convert(config: Node): Stage = {
    ...
    new ObjectStorageWriteStage(config.id, this, azureConfig, azureConfig.options ++ getOptions(config.value), AzureBlobStorageStageConfig.storageId)
  }
}
...
object GoogleCloudStorageWriteStageBuilder extends WriteStageBuilder {
  ...
  override protected def convert(config: Node): Stage = {
    ...
    new ObjectStorageWriteStage(config.id, this, gcpConfig, gcpConfig.options ++ getOptions(config.value), GoogleCloudStorageStageConfig.storageId)
  }
}
