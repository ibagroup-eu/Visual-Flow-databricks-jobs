...
import by.iba.vf.spark.transformation.stage.{ReadStageBuilder, SnowflakeStageConfig, Stage, StageBuilder}
import org.apache.spark.sql.{DataFrame, SparkSession}

class SnowflakeReadStage(override val id: String, config: SnowflakeStageConfig)
  extends ReadStage(id, SnowflakeStageConfig.storageId) {

  override def read(implicit spark: SparkSession): DataFrame =
    spark.read.format("net.snowflake.spark.snowflake").options(config.parameter).load()

  override val builder: StageBuilder = SnowflakeReadStageBuilder
}

object SnowflakeReadStageBuilder extends ReadStageBuilder {
  ...
  override protected def convert(config: Node): Stage =
    new SnowflakeReadStage(config.id, new SnowflakeStageConfig(config))
}
