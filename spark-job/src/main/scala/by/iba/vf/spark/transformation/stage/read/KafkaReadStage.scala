...
class KafkaReadStage(override val id: String, config: KafkaStageConfig, options: Map[String, String])
  extends ReadStage(id, KafkaStageConfig.storageId){
...
object KafkaReadStageBuilder extends ReadStageBuilder {
  ...
  override protected def convert(config: Node): Stage =
    new KafkaReadStage(config.id, new KafkaStageConfig(config), getOptions(config.value))
}