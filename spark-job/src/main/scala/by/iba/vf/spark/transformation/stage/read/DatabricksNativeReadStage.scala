...
class DatabricksNativeReadStage(override val id: String, config: DatabricksNativeStageConfig, options: Map[String, String])
  extends ReadStage(id, DatabricksNativeStageConfig.storageId){
...
object DatabricksNativeReadStageBuilder extends ReadStageBuilder {
  ...
  override protected def convert(config: Node): Stage =
    new DatabricksNativeReadStage(config.id, new DatabricksNativeStageConfig(config), getOptions(config.value))
}