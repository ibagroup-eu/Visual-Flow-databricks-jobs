...
class KafkaWriteStage(override val id: String, config: KafkaStageConfig, options: Map[String, String])
  extends WriteStage(id, KafkaStageConfig.storageId){
...
object KafkaWriteStageBuilder extends WriteStageBuilder {
  ...
  override protected def convert(config: Node): Stage =
    new KafkaWriteStage(config.id, new KafkaStageConfig(config), getOptions(config.value))
}