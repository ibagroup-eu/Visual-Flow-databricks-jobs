...
class SnowflakeWriteStage(override val id: String, config: SnowflakeStageConfig)
  extends WriteStage(id, SnowflakeStageConfig.storageId) {
...
object SnowflakeWriteStageBuilder extends WriteStageBuilder {
  ...
  override protected def convert(config: Node): Stage =
    new SnowflakeWriteStage(config.id, new SnowflakeStageConfig(config))
}
