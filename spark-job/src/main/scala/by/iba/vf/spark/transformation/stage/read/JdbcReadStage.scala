...
private[read] class JdbcReadStage(
  ...
  override def read(implicit spark: SparkSession): DataFrame = {
    ...
    val config = if (customSql && options.contains(JDBCOptions.JDBC_TABLE_NAME))
      ...
      jdbcConfig + (JDBCOptions.JDBC_TABLE_NAME -> fullyQualifiedTable)
...
object JdbcReadStageBuilder extends JdbcStageBuilder with ReadStageBuilder {
  ...
  override protected def convert(config: Node): Stage = {
    ...
    new JdbcReadStage(config.id, fullyQualifiedTable, truststorePathOption, map, getOptions(config.value), config.value(fieldCustomSQL).toBoolean)
  }
}
