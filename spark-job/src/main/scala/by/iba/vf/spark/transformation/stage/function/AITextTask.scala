...
private[function] final class AITextTaskStage(val id: String, config: AITextTaskStageConfig) extends Stage {
...
object AITextTaskStageBuilder extends StageBuilder {
  ...
  override protected def convert(config: Node): Stage =
    new AITextTaskStage(config.id, new AITextTaskStageConfig(config))
}
