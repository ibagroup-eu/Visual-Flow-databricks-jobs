...
object ObjectStorageReadS3StageBuilder extends ReadStageBuilder {
  ...
  override protected def convert(config: Node): Stage = {
    ...
    new ObjectStorageReadStage(config.id, this, s3Config, getOptions(config.value) ++ s3Config.options, S3Config.cosStorage)
  }
}
...
object AzureBlobStorageReadStageBuilder extends ReadStageBuilder {
  ...
  override protected def convert(config: Node): Stage = {
    ...
    new ObjectStorageReadStage(config.id, this, azureConfig, azureConfig.options ++ getOptions(config.value), AzureBlobStorageStageConfig.storageId)
  }
}
...
object GoogleCloudStorageReadStageBuilder extends ReadStageBuilder {
  ...
  override protected def convert(config: Node): Stage = {
    ...
    new ObjectStorageReadStage(config.id, this, gcpConfig, gcpConfig.options ++ getOptions(config.value), GoogleCloudStorageStageConfig.storageId)
  }
}
